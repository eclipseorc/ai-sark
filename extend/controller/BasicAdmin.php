...
class BasicAdmin extends Controller
{
    ...
    /**
     * 表单默认操作
     * @param null $dbQuery 数据库操作对象
     * @param string $tplFile   显示模板名
	...
	protected function _form($dbQuery = null, $tplFile = '', $pkField = '', $where = [], $extendData = [])
	{
		...
		if (!$this->request->isPost()) {
			$vo = ($pkValue !== null) ? array_merge($db->where($pk, $pkValue)->where($where)->find(), $extendData) : $extendData;
     ...
     * @param bool $isPage  是否启用分页
     * @param bool $isDisplay   是否直接显示输出
     * @param bool $total   总记录数
     * @param array $result 结果集
     * @return array|mixed
     */
	public function _list($dbQuery = null, $isPage = true, $isDisplay = true, $total = false, $result = [])
    {
        $db = is_null($dbQuery) ? Db::name($this->table) : (is_string($dbQuery) ? Db::name($dbQuery) : $dbQuery);
        //列表排序默认处理
        ...
        if ($isPage) {
            ...
            if (($totalNum = $page->total()) > 0) {
                ...
                $html = "<span class='pagination-trigger nowrap'>共 {$totalNum} 条记录，每页显示 <select data-auto-none>" . implode('', $rowHTML) . "</select> 条，共 {$maxNum} 页当前显示第 {$curPage} 页。</span>";
